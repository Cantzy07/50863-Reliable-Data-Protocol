...
if __name__ == '__main__':
	...
	window_size = int(min(50, (link_bandwidth * propagation_delay) // max_packet_size))
	# the amount of time it takes for a round trip for all packets sent assuming ack and data have max transmission time
	max_time_to_wait = window_size * (propagation_delay * 2 + transmission_delay * 2)
				...
				with lock:
					...
					if ack_buffer[i] is None:
						...
						if time.time() - ack_buffer[i][2] >= max_time_to_wait:
							...
							print("packet timeout, resending", ack_buffer[i][0])
